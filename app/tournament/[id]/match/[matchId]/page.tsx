...
export default function MatchPage() {
  ...
  const handleDartInput = async (baseValue: number) => {
    ...
    const scoredPoints = baseValue === 25 && mult === 'double' ? 50 : baseValue * (mult === 'single' ? 1 : mult === 'double' ? 2 : 3);
  ...
  const submitTurn = async (darts: Dart[], isBust: boolean, isCheckout: boolean, newRest: number) => {
    const isPlayerA = match.currentTurnId === match.playerAId;
    ...
    if (isPlayerA) {
      updates.playerADartsThrown = (match.playerADartsThrown || 0) + darts.length;
      if (!isBust) {
        updates.playerAScored = (match.playerAScored || 0) + turnScore;
        updates.playerARest = newRest;
      }
    } else {
      updates.playerBDartsThrown = (match.playerBDartsThrown || 0) + darts.length;
      if (!isBust) {
    ...
    if (matchEnded) {
      ...
      const legsNeededToWin = isBo3 ? 2 : 1;
      const matchFullyEnded = newPlayerALegs >= legsNeededToWin || newPlayerBLegs >= legsNeededToWin || isDraw;

      if (matchFullyEnded) {
        updates.status = 'completed';
        updates.playerALegs = newPlayerALegs;
        updates.playerBLegs = newPlayerBLegs;
        if (winnerId) updates.winnerId = newPlayerALegs > newPlayerBLegs ? match.playerAId : match.playerBId;
        ...
        updates.currentTurnId = updates.playerAStartsLeg ? match.playerAId : match.playerBId;
        updates.playerARest = match.format === '501_bo3' ? 501 : 301;
        updates.playerBRest = match.format === '501_bo3' ? 501 : 301;
        ...
        <Card className="w-full max-w-md">
          <CardContent className="p-6 text-center space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <Button size="lg" onClick={() => setStarter(match.playerAId)}>{match.playerAName}</Button>
              <Button size="lg" onClick={() => setStarter(match.playerBId)}>{match.playerBName}</Button>
            </div>
          </CardContent>
        </Card>
      </div>
  ...
  const avgA = match.playerADartsThrown ? ((match.playerAScored / match.playerADartsThrown) * 3).toFixed(2) : '0.00';
  const avgB = match.playerBDartsThrown ? ((match.playerBScored / match.playerBDartsThrown) * 3).toFixed(2) : '0.00';
  ...
  const renderPlayerRow = (isPlayerA: boolean) => {
    const isActive = match.currentTurnId === (isPlayerA ? match.playerAId : match.playerBId);
    const name = isPlayerA ? match.playerAName : match.playerBName;
    const rest = isPlayerA ? match.playerARest : match.playerBRest;
    const dartsThrown = isPlayerA ? (match.playerADartsThrown || 0) : (match.playerBDartsThrown || 0);
    ...
    if (isActive) {
      ...
      const playerTurns = (match.turns || []).filter((t: any) => t.playerId === (isPlayerA ? match.playerAId : match.playerBId));
        ...
        <div className="flex-1 p-4 flex flex-col justify-center border-r">
          <div className="flex justify-between items-center mb-1">
            <span className="text-sm text-zinc-500 flex items-center gap-1">
              <Target className="w-3 h-3" /> {dartsThrown + (isActive ? currentDarts.length : 0)}
            </span>
          </div>
          <div className="text-6xl font-bold tracking-tighter">{rest - (isActive ? turnTotal : 0)}</div>
          <div className="text-lg text-zinc-600 mt-1">{name}</div>
        </div>
        <div className="flex-[2] flex flex-col">
          <div className="flex flex-1 border-b">
            {[0, 1, 2].map(i => (
              <div key={i} className="flex-1 border-r flex items-center justify-center text-2xl font-medium">
                {displayDarts[i] ? formatDart(displayDarts[i]) : ''}
              </div>
            ))}
            <div className="flex-1 flex items-center justify-center text-2xl font-medium bg-zinc-50">
              {displayDarts.length > 0 ? turnTotal : ''}
            </div>
          </div>
          <div className="flex flex-1">
            <div className="flex-[3] border-r flex items-center justify-center text-3xl font-bold">
              {displayDarts.length > 0 ? turnTotal : ''}
            </div>
            <div className="flex-1 flex items-center justify-center text-xl text-zinc-600 bg-zinc-50">
              Ø {avg}
            </div>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-zinc-100 flex flex-col">
      <div className="bg-slate-800 text-white p-2 flex items-center justify-between shadow-md">
        ...
        <div className="text-sm font-medium">
          {match.format === '301_bo1' ? '301 (Best of 1)' : match.format === '301_bo3' ? '301 (Best of 3)' : '501 (Best of 3)'}
          {match.answerThrowActive && <span className="ml-4 text-orange-400 font-bold uppercase">Answer Throw Active!</span>}
        </div>
        ...
        {match.status === 'completed' ? (
          ...
          <div className="flex-1 bg-zinc-200 p-2 flex flex-col">
            <div className="text-center py-2 text-zinc-600 font-medium flex justify-between px-4">
              <span>Leg {match.currentLeg}, {match.currentTurnId === match.playerAId ? match.playerAName : match.playerBName} is throwing.</span>
              {getCheckoutSuggestion((match.currentTurnId === match.playerAId ? match.playerARest : match.playerBRest) - currentDarts.reduce((sum, d) => sum + d.scoredPoints, 0), 3 - currentDarts.length) && (
                <span className="text-green-700 font-bold">
                  Route: {getCheckoutSuggestion((match.currentTurnId === match.playerAId ? match.playerARest : match.playerBRest) - currentDarts.reduce((sum, d) => sum + d.scoredPoints, 0), 3 - currentDarts.length)}
                </span>
              )}
            </div>
            ...
            <div className="flex-1 grid grid-cols-7 gap-1 p-1">
              ...
              <Button 
                className={`h-full text-xl font-bold rounded-none col-span-2 ${multiplier === 'double' ? 'bg-orange-600 hover:bg-orange-700 text-white' : 'bg-orange-500 hover:bg-orange-600 text-white'}`}
                onClick={() => setMultiplier(multiplier === 'double' ? 'single' : 'double')}
              >
                DOUBLE
              </Button>
              <Button 
                className={`h-full text-xl font-bold rounded-none col-span-2 ${multiplier === 'triple' ? 'bg-red-700 hover:bg-red-800 text-white' : 'bg-red-600 hover:bg-red-700 text-white'}`}
                onClick={() => setMultiplier(multiplier === 'triple' ? 'single' : 'triple')}
              >
                TRIPLE
              </Button>
              <Button 
                className="h-full text-xl font-bold bg-slate-800 hover:bg-slate-900 text-white rounded-none col-span-2"
                onClick={handleUndo}
